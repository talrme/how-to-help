...
- The `Projects` section should default to a short faded preview with a `Show all projects` overlay, then show a tiny `Hide projects` link at the bottom when expanded. Settings includes a device-local `Show projects by default` toggle.
